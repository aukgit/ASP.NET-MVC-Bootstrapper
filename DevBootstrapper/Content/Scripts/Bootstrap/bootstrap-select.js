                ...
                if ($this.parent().is("optgroup") && $this.data("divider") !== true) {
                    if ($this.index() === 0) { // Is it the first option of the optgroup?
                        ...
                        if (index !== 0 && li.length > 0) { // Is it NOT the first option of the select && are there elements in the dropdown?
                            li.push(generateLi("", null, "divider", optId + "div"));
                        }

                        li.push(generateLi(label, null, "dropdown-header", optId));
                    }

                    li.push(generateLi(generateA(text, "opt " + optionClass, inline, tokens), index, "", optId));
                } else if ($this.data("divider") === true) {
                    li.push(generateLi("", index, "divider"));
                } else if ($this.data("hidden") === true) {
                    li.push(generateLi(generateA(text, optionClass, inline, tokens), index, "hidden is-hidden"));
                } else {
                    if ($this.prev().is("optgroup")) li.push(generateLi("", null, "divider", optId + "div"));
                    li.push(generateLi(generateA(text, optionClass, inline, tokens), index));
                }
            });
